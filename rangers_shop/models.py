from werkzeug.security import generate_password_hash #generates unique password hash for extra security
...
class Product(db.Model): #db.Model helps us translate python code to coulmns in SQL
    ...
    def set_image(self, image, name):
        ...
        if not image: #aka the user did not give us an image
            ...
            image = get_image(name) #name is going to become search in the helpers method & we are going to use it to make our api call
...
class Customer(db.Model):
    ...
    #this is how we tie a table to another on
    prodord = db.relationship('ProdOrder', backref = 'customer') #lazy = True means is we don't need the ProdOrder table to have a customer

    def __init__(self, cust_id):
        self.cust_id = cust_id #when a customer makes an order on the frontend they will pass us their cust_id

    def __repr__(self):
        return f"<Customer: {self.cust_if}>"
...
class ProdOrder(db.Model):
    ...
    #first instance of using a primary key as a foriegn key on THIS table
...
product_shcema = ProductSchema() #this is 1 singular product
products_shcema = ProductSchema(many=True) #bringing back all the products in our database & sending to frontend